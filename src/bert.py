...
def main():
    ...
    parser.add_argument("--save_path",
                        default='../save_model/dbp15k/zh_en/bert/',
                        type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--test_path",
                        default='../results/dbp15k/zh_en/bert/',
                        type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        train_bert_completion(args, REL_NUM, train_triples, bert_model, tokenizer, bert_optimizer, ent2text,
                                     id2ent,
                                     device)
        test_bert_completion(args, epoch, bert_model, tokenizer, train_triples, test_triples1, ent2text, id2ent, device,
                             "triples1")
        test_bert_completion(args, epoch, bert_model, tokenizer, train_triples, test_triples2, ent2text, id2ent, device,
                             "triples2")
        test_bert_completion(args, epoch, bert_model, tokenizer, train_triples, test_ills_triples, ent2text, id2ent, device,
                             "ills")
        if (epoch + 1) % 5 == 0:
            test_bert_completion(args, epoch, bert_model, tokenizer, train_triples, test_cross, ent2text, id2ent, device,
                                 "cross")
        if (epoch + 1) % 10 == 0:
            test_bert_completion(args, epoch, bert_model, tokenizer, train_triples, test_triples, ent2text, id2ent,
                                 device,
                                 "all")
